...
################################
#read ACIN voltage    79h, 78h    0 mV -> 000h,    1.7 mV/bit    FFFh -> 4.5045 V
BAT_VOLT_LSB=$(i2cget -y -f 0 0x34 0x57)
BAT_VOLT_MSB=$(i2cget -y -f 0 0x34 0x56)

#echo $BAT_VOLT_MSB $BAT_VOLT_LSB

BAT_BIN=$(( $(($BAT_VOLT_MSB << 4)) | $(($(($BAT_VOLT_LSB & 0xF0)) >> 4)) ))

BAT_VOLT=$(echo "$BAT_BIN*1.7" |bc)
echo Battery voltage = $BAT_VOLT mV 

###################
#read ACIN Discharge Current    7Ah, 7Bh    0 mV -> 000h,    0.625 mA/bit    FFFh -> 4.095 V
BAT_IDISCHG_LSB=$(i2cget -y -f 0 0x34 0x59)
BAT_IDISCHG_MSB=$(i2cget -y -f 0 0x34 0x58)

#echo $BAT_IDISCHG_MSB $BAT_IDISCHG_LSB

BAT_IDISCHG_BIN=$(( $(($BAT_IDISCHG_MSB << 4)) | $(($(($BAT_IDISCHG_LSB & 0xF0)) >> 4)) ))

BAT_IDISCHG=$(echo "$BAT_IDISCHG_BIN*0.625" |bc)
...
echo Currrent CPU Freq $(cat /sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_cur_freq)